...
2. **Branch off `main` in a new worktree.** Never commit to `main`. Name the branch `agent/<issue#>-<slug>`, e.g. `agent/1-walking-skeleton`, and create it in its own worktree so several implementations can proceed in parallel: `git worktree add ../garuff-<issue#>-<slug> -b agent/<issue#>-<slug> main`. Do the rest of the work from that worktree directory. garuff holds no runtime state, so worktrees never collide.
3. **Work test-first.** Use the `tdd` skill (red → green → refactor); see [testing.md](./testing.md) for what to test, the `tmp_path` fixture-project pattern, and the boundary rules.
